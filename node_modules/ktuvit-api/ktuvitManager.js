...
class KtuvitManager {
   ...
   * @param {string} baseUrl Optional base URL for Ktuvit API
   */
  constructor(loginCookie, useCache = true, baseUrl = null) {
    // Allow override via environment variable or constructor parameter
    this.BASE_URL = baseUrl || process.env.KTUVIT_BASE_URL || "https://www.ktuvit.me/";
  ...
  async getSubsIDsListEpisode(ktuvitID, season, episode) {
    ...
    var res = await this.getWithLoginInfo(
      this.KTUVIT.EPISODE_INFO_URL + query_string
    );
  ...
  async getSubsIDsListMovie(ktuvitID) {
    var res = await this.getWithLoginInfo(
      this.KTUVIT.MOVIE_INFO_URL + ktuvitID
    );
    var subtitles = this.extractSubsFromHtml(res.text);
    return subtitles || [];
  }
  ...
  async downloadSubtitle(KtuvitId, subId, cb, extraOpts) {
    ...
    let downloadIdentifier = await this.postWithLoginInfo(
      this.KTUVIT.REQUEST_DOWNLOAD_IDENTIFIER_URL,
    ...
    await superagent
      .get(this.KTUVIT.DOWNLOAD_SUB_URL + downloadIdentifier)
